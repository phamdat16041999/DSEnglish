...
def addVocabularyByExtention(request):
	if request.user.is_authenticated:
		CategoryID = Category.objects.get(UserID = request.user.id, Name = "Google chrome extention").id
...
def createCategoryInterface(request):
	if request.user.is_authenticated:
		 return render(request, 'user/createCategory.html')
	else:
		return render(request, 'login.html')
...
def chooseGame(request, id):
	if request.user.is_authenticated:
		...
		return render(request, 'user/Vocabulary.html', newWord)
	else:
		return render(request, 'login.html')
...
def deleteVocabulary(request, id):
	if request.user.is_authenticated:
		with connection.cursor() as cursor:
			cursor.execute(
			    "DELETE FROM index_vocabulary WHERE index_vocabulary.id = (SELECT index_vocabulary.id FROM index_vocabulary INNER JOIN index_category on index_vocabulary.CategoryID_id = index_category.id INNER JOIN auth_user ON auth_user.id = index_category.UserID_id WHERE auth_user.id = '%s' AND index_vocabulary.id = '%s')" ,
			    [request.user.id,id]
			)
		return render(request, 'user/Index.html')
	else:
...
def updateVocabulary(request, id, Term, Definition):
	if request.user.is_authenticated:
		with connection.cursor() as cursor:
			cursor.execute(
			    "UPDATE index_vocabulary SET index_vocabulary.Term = %s, index_vocabulary.Definition =  %s WHERE index_vocabulary.id = (SELECT index_vocabulary.id FROM index_vocabulary INNER JOIN index_category on index_vocabulary.CategoryID_id = index_category.id INNER JOIN auth_user ON auth_user.id = index_category.UserID_id WHERE auth_user.id = '%s' AND index_vocabulary.id = '%s')" ,
